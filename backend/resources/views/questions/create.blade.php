        ...
        <div class="row mb-4">
            <div class="col-md-8">
                <div class="card p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <textarea name="text" id="text" class="form-control"
                                placeholder="Enter your question here..." required value="{{ old('text') }}"></textarea>
                            <button type="button" id="rephraseButton" class="btn btn-secondary" data-bs-toggle="tooltip"
                                title="Click to rephrase the question">
                    ...
                    @for ($i = 0; $i < 4; $i++) <div class="row mb-3 align-items-center">
                        <div class="col-10">
                            <label class="form-label" for="answers[{{ $i }}][text]">Choice {{ $i + 1 }}:</label>
                            <input type="text" name="answers[{{ $i }}][text]"
                                class="form-control @error('answers.' . $i . '.text') is-invalid @enderror"
                                placeholder="Enter choice {{ $i + 1 }}..." value="{{ old('answers.' . $i . '.text') }}"
                                required>
                        ...
                        <div class="col-2 text-center">
                            <div class="form-check">
                                <input type="radio" name="correct_answer" value="{{ $i }}"
                                    class="form-check-input custom-radio" {{ old('correct_answer', 0)==$i ? 'checked'
                                    : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (correctAnswer) {
            ...
            if (correctAnswerIndex !== -1) {
                const correctAnswerRadio = document.querySelector(`input[name="correct_answer"][value="${correctAnswerIndex}"]`);
